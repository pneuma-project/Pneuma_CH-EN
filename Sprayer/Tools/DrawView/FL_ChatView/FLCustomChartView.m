...
@interface FLCustomChartView ()<UIScrollViewDelegate>
{
...
@implementation FLCustomChartView
...
-(instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        self.yScaleArr = [NSArray array];
        self.xScaleArr = [NSArray array];
...
-(void)setDataArrOfX:(NSArray *)dataArrOfX{
    ...
    if (_yDataArr.count) {
        [self addDataPointWith:self.scrollBgView1 andyArr:_yDataArr andxArr:_xDataArr];//添加点
...
- (void)setYDataArr:(NSArray *)yDataArr {
    _yDataArr = yDataArr;
...
//    }
    for (NSUInteger index = 1; index < self.leftPointArr.count - 2; index++) {
        ...
        if (index == self.leftPointArr.count-3) {
            [beizer moveToPoint:p3];
            lastPoint = p3;
        }
...
#pragma mark - 创建坐标点
-(void)addDataPointWith:(UIView *)view andyArr:(NSArray *)yData andxArr:(NSArray *)xData{
    
    self.yScaleArr = yData;
    self.xScaleArr = xData;
    
    CGFloat height = self.bgView1.bounds.size.height;
    CGFloat width = self.bgView1.bounds.size.width;
    
    //初始点
    NSMutableArray *xArr = [NSMutableArray arrayWithArray:xData];
    [xArr insertObject:@"0" atIndex:0];
    float tempXMax = [xArr[0] floatValue];
    for (int i = 1; i < xArr.count; ++i) {
        if ([xArr[i] floatValue] > tempXMax) {
            tempXMax = [xArr[i] floatValue];
        }
    }
    float xMax = [self.dataArrOfX[0] floatValue];
    for (int i = 1; i< self.dataArrOfX.count; ++i) {
        if ([self.dataArrOfX[i] floatValue] > xMax) {
            xMax = [self.dataArrOfX[i] floatValue];
        }
    }
    
    //初始点
    NSMutableArray *arr = [NSMutableArray arrayWithArray:yData];
    ...
    for (int i = 0; i<xArr.count; i++) {
        ...
        float tempWidth = [xArr[i] floatValue]/xMax;
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(width*tempWidth, height*(1 - tempHeight)-(btnW/2), btnW, btnW)];
