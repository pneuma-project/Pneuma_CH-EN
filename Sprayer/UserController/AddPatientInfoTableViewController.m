...
-(void)saveClick
{
    ...
    NSString * sql = [NSString stringWithFormat:@"insert into userInfo(name,relationship,sex,age,race,height,weight,phone,device_serialnum,isselect,trainData,macAddress) values('%@','%@','%@','%@','%@','%@','%@','%@','%@',%ld,'%@');",model.name,model.relationship,model.sex,model.age,model.race,model.height,model.weight,model.phone,model.deviceSerialNum,model.isSelect,nil,nil];
